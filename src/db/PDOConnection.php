...
use PDO;
use PDOStatement;
use syx6\db\exception\BindParamException;
use syx6\db\exception\DataNotFoundException;
use syx6\db\exception\ModelNotFoundException;
use syx6\db\exception\PDOException;
...
abstract class PDOConnection extends Connection
{
    ...
    {
        return $this->getConfig('builder') ?: '\\syx6\\db\\builder\\' . ucfirst($this->getConfig('type'));
    }
    ...
    public function aggregate(BaseQuery $query, string $aggregate, $field, bool $force = false)
    {
        ...
        $field = $aggregate . '(' . (!empty($distinct) ? 'DISTINCT ' : '') . $this->builder->parseKey($query, $field, true) . ') AS syx6_' . strtolower($aggregate);
    ...
    protected function getCacheKey(BaseQuery $query): string
    {
        if (!empty($query->getOptions('key'))) {
            $key = 'syx6:' . $this->getConfig('database') . '.' . $query->getTable() . '|' . $query->getOptions('key');
        } else {
